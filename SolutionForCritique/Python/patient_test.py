...
class PatientTest(unittest.TestCase):
    ...
    def test_no_clash_when_no_overlap(self):
        ...
        self.prozac.add_prescription(Prescription(dispense_date=(date.today() - timedelta(days=90)), days_supply=30))
    ...
    def test_clash_when_medicines_taken_continuously(self):
        ...
        self.prozac.add_prescription(Prescription(dispense_date=(date.today() - timedelta(days=30)), days_supply=30))
        self.prozac.add_prescription(Prescription(dispense_date=(date.today() - timedelta(days=60)), days_supply=30))
        self.prozac.add_prescription(Prescription(dispense_date=(date.today() - timedelta(days=90)), days_supply=30))
    ...
    def test_two_medicines_taken_overlapping_current_date(self):
        ...
        self.assertEquals(set([date.today() - timedelta(days=1)]), self.patient.clash(["Codeine", "Prozac"], 90))

    def test_two_medicines_taken_overlapping_start_of_period(self):
        self.codeine.add_prescription(Prescription(dispense_date=(date.today() - timedelta(days=91)), days_supply=30))
        self.prozac.add_prescription(Prescription(dispense_date=(date.today() - timedelta(days=119)), days_supply=30))
        self.assertEquals(set([date.today() - timedelta(days=90)]), self.patient.clash(["Codeine", "Prozac"], 90))
